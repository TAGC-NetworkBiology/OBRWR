...
from obrwr import solution_parser as sparse
...
class MyGraph:
    ...
    def control_optimization(self,eps,beta):
        if self.optimized:
            ...
            sns.scatterplot(x=np.log(tmp),y=np.log(self.pi),alpha=.5, s=60)
    ...
    def find_eps_critic(self,beta,sensitivity):
        a,b = 1e-4,1-1e-4
        d = self.optimize_biased_walk_normalized(b,beta)[4].getObjectiveValue()
        while b-a > sensitivity:
            m = self.optimize_biased_walk_normalized((a+b)/2,beta)[4].getObjectiveValue()
    ...
    def optimize_biased_walk_normalized(self,eps,beta,OF=0,objective=OBJ_VAR_FROM_STABLE,name="test"):
        ...
        indices_W = np.split(self.W_sparse.indices, self.W_sparse.indptr)[1:-1]
        indices_inM = np.split(self.inM.indices,self.inM.indptr)[1:-1]
        
        self.mo.indices_W = pyo.Set(within=self.mo.N*self.mo.N,initialize=[(i,j) for i in range(self.N) for j in indices_W[i] ])
        self.mo.W_sparse = pyo.Param(self.mo.indices_W,initialize=lambda m,i,j:self.W_sparse[i,j])
        
        self.mo.indices_inM = pyo.Set(within=self.mo.N*self.mo.m,initialize=[(i,j) for j in range(self.m) for i in indices_inM[j] ])
        self.mo.inM = pyo.Param(self.mo.indices_inM,initialize=lambda m,i,j:self.inM[i,j])
        def rule_constr_4(model,i):
             return sum(model.pi[j]*self.W_sparse[i,j] for j in model.N if (i,j) in model.indices_W )*(1-eps)*(1-beta) + \
                 sum(model.E[j]*self.inM[i,j] for j in model.E if (i,j) in model.indices_inM)*eps*(1-beta) + beta*self.pr[i] == model.pi[i]
        ...
        self.mo.c4 = pyo.Constraint(self.mo.N,rule=rule_constr_4)
        ...
        self.mo.indices_outM = pyo.Set(within=self.mo.N*self.mo.m,initialize=[(i,j) for j in range(self.m) for i in indices_outM[j] ])
        self.mo.outM = pyo.Param(self.mo.indices_outM,initialize=lambda m,i,j:self.outM[i,j])
        def rule_constr_5(model,i):
            return sum(model.E[j]*model.outM[i,j] for j in model.m if (i,j) in model.indices_outM) == model.pi[i]
