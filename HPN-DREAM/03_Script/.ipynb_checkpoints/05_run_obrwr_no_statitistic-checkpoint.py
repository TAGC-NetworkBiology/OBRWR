...
def build_network(file_name):
    ...
    myGraph.set_sources([activator],1e6)
    return myGraph,G
...
def finalize_optim_and_save(myGraph):
    ...
    myGraph.add_meta_after_optim(eps_crit,beta, activator)
...
if __name__ == '__main__':
    ...
    for cl in cell_lines:
        for act in list_of_activators:
            l.append('_'.join([cl,act]))
    for file_name in [input_folder + "stimuli/top_tables_novoom/" + el + sensitive_proteins_file for el in l]:
